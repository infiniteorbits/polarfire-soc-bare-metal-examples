...
#define DCT_EXTRA_CHECKS
        ...
        case DDR_TRAINING_FINISH_CHECK:
            ...
            {
                tip_register_status (g_debug_uart);
                uprint(g_debug_uart, "\n\r\n\r DDR Training pass was successful");
#ifdef DEBUG_DDR_DDRCFG
                debug_read_ddrcfg();
#endif
 ...
 */
#ifdef MANUAL_ADDCMD_TRAINIG
static uint8_t g_offsets_sorted = 0u;
...
static uint8_t ddr_manual_addcmd_refclk_offset(DDR_TYPE ddr_type, uint8_t * refclk_sweep_index)
{
    ...
    type_array_index = 2U;
    if (g_offsets_sorted == 0u)
    {
        g_offsets_sorted = 1u;

        uint8_t arr[LIBERO_SETTING_REFCLK_DDR4_1600_NUM_OFFSETS] = {0};
        for (uint8_t i = 0; i < LIBERO_SETTING_REFCLK_DDR4_1600_NUM_OFFSETS; ++i)
        {
            arr[i] = REFCLK_OFFSETS[type_array_index][i+1];
        }
