...
print(const uint8_t *const message, mss_uart_instance_t* uart)
{
    MSS_UART_polled_tx_string(uart, (const uint8_t*)" "); /// Indent
    MSS_UART_polled_tx_string(uart, (const uint8_t*)message);
}
...
RFIM_Phy_write_reg(uint32_t base_addr, uint16_t phy_addr, uint32_t reg_addr, uint16_t reg_val)
{
    ...
    /// Check the network status for transaction completition
    ///
    do
    {
        --timeout;
...
RFIM_Phy_autonegotiate(uint32_t base_addr, uint8_t phy_addr)
{
    ...
    volatile uint16_t autoneg_complete;

    volatile int32_t   copper_aneg_timeout = 100000;
...
RFIM_Phy_loopback_autonegotiate(uint32_t base_addr0, uint8_t phy_addr0, uint32_t base_addr1, uint8_t phy_addr1)
{
    ...
    volatile int32_t copper_aneg_timeout = 100000u;
    volatile int32_t mii_aneg_timeout = 100000u;
...
RFIM_Phy_dump_id(uint32_t base_addr, uint32_t phy_addr, mss_uart_instance_t* uart)
{
    ...
    if (phy_id_1 == 0x0007) {
        if (phy_id_2 == 0x0771) {
            ...
            {
            case 0:
                print("GEM 0 PHY Model vsc8541-01 rev b\n\r", uart);
                break;
            case 1:
                print("GEM 1 PHY Model vsc8541-01 rev b\n\r", uart);
                break;
            }
            return;
        }
        if (phy_id_2 == 0x0772) {
            ...
            {
            case 0:
                print("GEM 0 PHY Model vsc8541-02/-05 rev c\n\r", uart);
                break;
            case 1:
                print("GEM 1 PHY Model vsc8541-02/-05 rev c\n\r", uart);
                break;
            }
            return;
        }
    ...
    {
    case 0:
        print("GEM 0 PHY Unknown model\n\r", uart);
        break;
    case 1:
        print("GEM 1 PHY Unknown model\n\r", uart);
        break;
    }
}
...
RFIM_Phy_dump_link_status(uint32_t base_addr, uint32_t phy_addr, mss_uart_instance_t* uart)
{
    ...
    print("link up:", uart);
    print(link  == PHY_LINK_UP ? "yes" : "no", uart);
    print(", speed:", uart);
    print(speed == PHY_MAC10MBPS ? "10 Mbps" : (speed == PHY_MAC100MBPS ? "100 Mbps" : (speed == PHY_MAC1000MBPS ? "1000 Mbps" : "Unknown")), uart);
    print("full duplex:", uart);
    print(link == PHY_LINK_DOWN ? "Unknown" : duplex == PHY_FULL_DUPLEX ? "yes" : "no", uart);
...
MSS_MAC_RFIM_phy_get_link_status(/* mss_mac_instance_t */ const void *v_this_mac, mss_mac_speed_t *speed, uint8_t *fullduplex)
{
    ...
    *fullduplex = PHY_FULL_DUPLEX;
    ...
    uint8_t link_up = RFIM_Phy_get_link_status(base_addr, this_mac->phy_addr, &phy_speed, fullduplex);
