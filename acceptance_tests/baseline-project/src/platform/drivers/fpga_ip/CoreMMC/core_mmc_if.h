...
/**
  The `cif_core_mmcresponse` type specifies the status of the MMC command transfer
  to the eMMC device. A value of this type is returned by `cif_core_mmcsend_core_mmc_cmd()`.
 */
...
/**
  `cif_core_mmcsend_core_mmc_cmd()` handles the interface level command and response
  ...
  @return
  This function returns a value of `cif_core_mmcresponse_t` type representing the
  ...
  @code
    ...
    response_status = cif_core_mmcsend_core_mmc_cmd(BASE_ADDR, MMC_DW_CSD, MMC_CMD_SWITCH)

    while(DEVICE_BUSY == response_status)
    {
        response_status = cif_core_mmcsend_core_mmc_cmd(BASE_ADDR,
                                       RCA_VALUE,
                                       MMC_CMD_SEND_STATUS)
    }
  @endcode
 */
cif_core_mmcresponse_t cif_core_mmcsend_core_mmc_cmd(addr_t addr, uint32_t cmd_arg, uint8_t cmd_type);

/**
  `send_core_mmc_cmd()` transfers the MMC bus command and argument to the MMC device
  ...
  @endcode
 */
void send_core_mmc_cmd(addr_t addr, uint32_t cmd_arg, uint8_t cmd_type,
