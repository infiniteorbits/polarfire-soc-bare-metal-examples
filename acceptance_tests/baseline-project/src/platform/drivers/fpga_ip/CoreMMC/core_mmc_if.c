 ...
 */
static cif_core_mmcresponse_t response_1_parser(addr_t addr);

/**
 * cif_core_mmcsend_core_mmc_cmd()
...
cif_core_mmcresponse_t cif_core_mmcsend_core_mmc_cmd(addr_t addr, uint32_t cmd_arg, uint8_t cmd_type) {
  cif_core_mmcresponse_t ret_status = TRANSFER_IF_FAIL;

  /* Make sure flags cleared */
  HAL_set_32bit_reg(addr, ICR, CLEAR_ALL_32);

  /* Transfer the Command to the MMC device */
  send_core_mmc_cmd(addr, cmd_arg, cmd_type, CHECK_IF_CMD_SENT_POLL);
