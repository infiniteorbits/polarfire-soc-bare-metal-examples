...
void calculate_and_print(unsigned int vbus_value, char* str, char* format, unsigned int factor) {
    // Calculate the numerator and denominator multiplication factor
    // Since we are doing integer division, we'll keep track of the decimal part separately
    unsigned long long numerator = (unsigned long long)vbus_value * factor * 100; // Multiplying by 100 to keep two decimal places
    unsigned int denominator = 65535;

    // Perform the division
    unsigned long long result = numerator / denominator;

    // Extract the integer part and the fractional part (last two digits)
    unsigned int integer_part = result / 100;
    unsigned int fractional_part = result % 100;

    sprintf(str, format, integer_part, fractional_part);
}
...
void PAC1934_drawISense(void)
{
    ...
    calculate_and_print(VBus[0u], (char*)&g_ui_buf[0], "    %04u.%03uma ", 1000);
    calculate_and_print(VBus[1u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "   %04u.%03uma ", 1000);
    calculate_and_print(VBus[2u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "    %04u.%03uma", 1000);
    calculate_and_print(VBus[3u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "      %04u.%03uma ", 1000);

    sprintf((char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "%s", "   ");
...
void PAC1934_drawVB(void)
{
    ...
    calculate_and_print(VBus[0u], (char*)&g_ui_buf[0], "     %02u.%02uv    ", 32);
    calculate_and_print(VBus[1u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "     %02u.%02uv    ", 32);
    calculate_and_print(VBus[2u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "     %02u.%02uv    ", 32);
    calculate_and_print(VBus[3u], (char*)&g_ui_buf[strlen((char*)&g_ui_buf[0])], "     %02u.%02uv    ", 32);
