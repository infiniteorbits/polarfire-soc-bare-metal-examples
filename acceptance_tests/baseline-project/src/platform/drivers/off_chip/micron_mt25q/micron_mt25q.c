...
#define MICRON_READ_FLAG_STATUS_REG             0x07
...
#define MICRON_CLR_FLAG_STATUS_REG              0x30
...
uint8_t
Flash_64KByte_erase(uint32_t addr, uint32_t len)
{
    uint8_t status = 0u;
    uint8_t command_buf[300] __attribute__ ((aligned (4))) = {0};
    uint32_t block_number = len >> 16;        // length / 65536
    uint32_t block = 0;
...
//    QSPI_TRANSFER_BLOCK(0, command_buf, 0, (uint8_t*)0, 0,0);
    ...
    do {
        ...
        command_buf[4] = addr & 0xFFu;
        command_buf[0] = MICRON_4BYTE_SECTOR_ERASE;   // erase one 64KByte Block
        ...
        addr = addr + 64*1024;  // increase target address
